...
import 'package:users_app/asistantMethods/cart_methods.dart';
...
class _CartScreenState extends State<CartScreen>
{
  ...
  void initState() {
   ...
   void initData()async{
     ...
     itemIDs= await cartMethods.separateItemIDsFromUserCartList();
    if(itemIDs != null) {
      itemQuantityList =
      await cartMethods?.separateItemQuantitiesFromUserCartList();
      setState(() {
              ...
              if(dataSnapshot.hasData)
              {
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate((context, index)
                  {
                    ...
                    double parsedPrice = double.tryParse(model.price!) ?? 0.0;
                    if(index == 0)
                    {
                      totalAmount = 0;
                      totalAmount = totalAmount + (parsedPrice * (itemQuantityList![index] ?? 0));
                    }
